...
import cn.crap.service.tool.PickService.op;
...
public class UserPickService implements IPickService{
	...
	enum op{ CATEGORY,MYMODULE,PROJECT_MODULE,MODULES,PROJECTTYPE,USER}
...
//						}
//						return;
					else if("MYMODULE".equals(code)){// 用户所有模块
						// 普通用户，只能查看自己的模块
						for (Project p : projectService.findByMap(Tools.getMap("userId", Tools.getUser().getId()), null, null)) {
						...
						return;
					}else if("PROJECTTYPE".equals(code)){
						//case PROJECTTYPE:
