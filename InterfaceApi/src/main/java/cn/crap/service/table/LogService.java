...
public class LogService extends BaseService<Log>
	...
	enum op{INTERFACE, ARTICLE, MODULE, PROJECT,SOURCE} 
	
	/*@Override*/
	@Transactional
	public void recover(Log log) throws MyException{
		log = get(log.getId());
		//op c=cn.crap.utils.EnumUtils.valueOf(op.class, log.getModelClass().toUpperCase());
		String code=log.getModelClass().toUpperCase();
		/*switch(c){*/
			JSONObject json = JSONObject.fromObject(log.getContent());
			if("INTERFACE".equals(code)){//恢复接口
			...
			}else if("MODULE".equals(code)){//恢复模块
				json = JSONObject.fromObject(log.getContent());
